...
%nterm <struct attrib> translation_unit
                       struct_specifier
                       function_definition
                       declaration_list
                       fun_declarator
...
statement: 
         ...
         | RETURN expression ';'
         {
            ...
            else if(ret == "int" && expType == "float") {
               op_unary_astnode* conv = new op_unary_astnode();
               conv->op = new std::string("TO_INT");
               conv->exp = obj->retExp;
               obj->retExp = (exp_astnode *) conv;
            }
            else if(ret == "float" && expType == "int") {
               op_unary_astnode* conv = new op_unary_astnode();
               conv->op = new std::string("TO_FLOAT");
               conv->exp = obj->retExp;
               obj->retExp = (exp_astnode *) conv;
            }
            else {
                  ...
                  if(baseType(t1) == "void") {
                     ...
                     if(depth(t2) != 0) {
                        ...
                        if(t2 == "float") {
                           ...
                           conv->op = new std::string("TO_INT");
                           conv->exp = exp;
                           obj->callExps[i] = (exp_astnode *) conv;
                        }
                        else if(t2 != t1) {
                           // Invalid
...
expression:
         ...
         | expression OR_OP logical_and_expression
         {
            ...
            bool c1 = isInt($1.type) || $1.type == "float";
            bool c2 = isInt($3.type) || $3.type == "float";
...
equality_expression:
         ...
         | equality_expression EQ_OP relational_expression
         {
            ...
            else if(isInt(t1) && t2 == "float") {
               op_unary_astnode* conv = new op_unary_astnode();
               conv->op = new std::string("TO_FLOAT");
               conv->exp = obj->lExp;
               obj->lExp = (exp_astnode *) conv;
...
additive_expression:
         ...
         | additive_expression '+' multiplicative_expression
         {
            ...
            if(!(t1 == "int" || t1 == "float") && !(t2 == "int" || t2 == "float")) {
               ...
               if(t1 == "float" || t2 == "float") $$.type = "float";
               else $$.type = "int";
            }
...
postfix_expression:
         ...
         | postfix_expression INC_OP
         {
            ...
            if((isInt($1.type) && !isArray($1.type)) || $1.type == "float") {
               // Good
            }
            else {
...
primary_expression:
         ...
         | FLOAT_CONSTANT
         {
            floatconst_astnode* obj = new floatconst_astnode();
         ...
         | STRING_LITERAL
         {
            string_astnode* obj = new string_astnode();
            obj->str = new std::string($1);
            $$.ast = obj;
            $$.text = $1;
            $$.type = "string";
            $$.lval = false;
